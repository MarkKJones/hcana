...
Int_t THcCoinTime::Process( const THaEvData& evdata )
{
  ...
  THaTrack* theHadTrack =(fhadSpectro->GetGoldenTrack()); 
  THaTrack* theElecTrack = (felecSpectro->GetGoldenTrack());
  ...
  if (felecSpectro->GetParticleMass() > 0.00052) return 1;
	  ...
	  Double_t SHMS_FPtime=elec_FPtime;
	  Double_t HMS_FPtime=had_FPtime;
	  if (felecArmName=="H") {
             ElecPathLength=HMScentralPathLen +  DeltaHMSpathLength;
	     HadPathLength=SHMScentralPathLen +  DeltaSHMSpathLength;
	     SHMS_FPtime=had_FPtime;
	     HMS_FPtime=elec_FPtime;
	  }
	  ...
	  //PROTON
	  fROC1_epCoinTime = fROC1_RAW_CoinTime - elec_coinCorr + had_coinCorr_proton - eHad_CT_Offset; 
	  fROC2_epCoinTime = fROC2_RAW_CoinTime - elec_coinCorr + had_coinCorr_proton - eHad_CT_Offset;       

	  //KAON
	  fROC1_eKCoinTime = fROC1_RAW_CoinTime - elec_coinCorr + had_coinCorr_Kaon - eHad_CT_Offset;
	  fROC2_eKCoinTime = fROC2_RAW_CoinTime - elec_coinCorr + had_coinCorr_Kaon - eHad_CT_Offset;
	
	  //PION
	  fROC1_ePiCoinTime = fROC1_RAW_CoinTime - elec_coinCorr + had_coinCorr_Pion - eHad_CT_Offset;	  
	  fROC2_ePiCoinTime = fROC2_RAW_CoinTime - elec_coinCorr + had_coinCorr_Pion - eHad_CT_Offset;

	  //POSITRON
	  fROC1_ePosCoinTime = fROC1_RAW_CoinTime - elec_coinCorr + had_coinCorr_Positron - eHad_CT_Offset ;	  
	  fROC2_ePosCoinTime = fROC2_RAW_CoinTime - elec_coinCorr + had_coinCorr_Positron - eHad_CT_Offset;
